 ...
 */
object ASTConverter {
  def apply(source: List[Stmt]): List[AnnotStmt] = {
    convert(source)
  }

  /* Convert functions */
  def convert(asts: List[Stmt]): List[AnnotStmt] = {
    var scopes: List[Scope] = new Scope() :: ScalaBase.getScope() :: Nil;

    for (ast <- asts) yield convertStmt(ast, scopes); // TODO does this handle mutual recursion properly at top level?
  }
  def convertStmt(stmt: Stmt, scopes: List[Scope]): AnnotStmt = stmt match {
    case EmptyStmt => AnnotEmptyStmt
  ...
  def convertExpr(stmt: Stmt, scopes: List[Scope]): AnnotExpr = stmt match {
    ...
    case AssignExpr(lhs, op, rhs) => convertAssignExpr(lhs, op, rhs, scopes);
  ...
  def convertAssignExpr(lhs: Expr, op: String, rhs: Expr, scopes: List[Scope]): AnnotAssignExpr = {
    var annotLhs: AnnotExpr = convertExpr(lhs, scopes);
    var annotRhs: AnnotExpr = convertExpr(rhs, scopes);
    return AnnotAssignExpr(annotLhs, op, annotRhs);
  }
