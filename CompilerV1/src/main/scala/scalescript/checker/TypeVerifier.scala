...
object TypeVerifier {
    ...
    def verifyBinOpExpr(op: String, l: AnnotExpr, r: AnnotExpr, scopes: List[Scope]): TypedBinOpExpr = {
        ...
        if (typeL != typeR) {
            if ((op == "+") && (typeL == BaseType(List("String")) || typeR == BaseType(List("String")))) {
                retType = BaseType(List("String"))
            ...
            op match {
                case ">=:" | "<=:" | ">:" | "<:" => retType = BaseType(List("Boolean"))
                case ">=" | "<=" | ">" | "<" => retType = BaseType(List("Boolean"))
                case "==:" | "!=:" => retType = BaseType(List("Boolean"))
                case "==" | "!=" => retType = BaseType(List("Boolean"))
    ...
    def verifyIfThenExpr(predicate: AnnotExpr, expr: AnnotExpr, scopes: List[Scope]): TypedIfThenExpr = {
        ...
        if (predicateTyped.evalType() != BaseType(List("Boolean"))) {
            throw new Exception("If statements require a predicate of type Boolean");
        } else {
            var commonType: Type = firstCommonSuperType(exprTyped.evalType(), BaseType(List("Unit")), scopes);
    ...
    def verifyWhileExpr(predicate: AnnotExpr, body: AnnotExpr, doFlag: Boolean, scopes: List[Scope]): TypedWhileExpr = {
        ...
        if (predicateTyped.evalType() != BaseType(List("Boolean"))) {
            ...
            return TypedWhileExpr(predicateTyped, bodyTyped, doFlag, BaseType(List("Unit")));
        }
    }
    ...
    def verifyStringExpr(str: String, scopes: List[Scope]): TypedStringExpr = {
        return TypedStringExpr(str, BaseType(List("String")))
    }
    def verifyCharExpr(ch: Char, scopes: List[Scope]): TypedCharExpr = {
        return TypedCharExpr(ch, BaseType(List("Char")))
    }
    def verifyNumExpr(num: Numeric, scopes: List[Scope]): TypedNumExpr = {
        var retType: Type = null;
        num match {
            case NInt(num) => retType = BaseType(List("Int"))
            case NDouble(num) => retType = BaseType(List("Double"))
        }
        if (retType == null) {
    ...
    def verifyBoolExpr(bool: Boolean, scopes: List[Scope]): TypedBoolExpr = {
        return TypedBoolExpr(bool, BaseType(List("Boolean")))
    }
    ...
    def verifyAssignExpr(lhs: AnnotExpr, rhs: AnnotExpr, scopes: List[Scope]): TypedAssignExpr = {
        ...
        return TypedAssignExpr(lhsTyped, rhsTyped, BaseType(List("Unit")))
    }
