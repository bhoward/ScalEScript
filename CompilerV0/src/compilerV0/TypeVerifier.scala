...
object TypeVerifier {
	...
	def processVerifyStack(maps : List[Map[String, Type]]) : List[String] = {
	...
	def verify(ast : Stmt, maps : List[Map[String, Type]]) : String = ast match {
		...
		case BlockExpr(listofstatements) => {
			...
			var stmts = listofstatements;
	  		var typedStmts : List[String] = Nil;
	  		while (stmts.length > 0){
	  			...
	  			stmt match {
	  			  	...
	  			  	case _ => {
	  			  		...
	  			  		typedStmts = verify(stmt, myMaps) :: typedStmts
	  			  	}
	  			}
	  		}
	  		...
	  		if (typedStmts.length > 0) {
		  		var lastStmtType = typedStmts.last;
		  		if (lastStmtType == "") {
		  			return "Unit"
		  		} else {
		  			return lastStmtType
	  	...
	  	case IfThenExpr(predicate, expr) => {
	  		...
	  		if (predicateType != "Boolean") {
	  			...
	  			return firstCommonSuperTypeS(exprType, "Unit")
	  		}
	  	}
  	    case IfThenElseExpr(predicate, trueValue, falseValue) => {
	  		...
	  		if (predicateType != "Boolean") {
	  			...
	  			var commonType = firstCommonSuperTypeS(trueType, falseType);
	  			if (commonType == "") {
	  	...
	  	case ValDefStmt(listOfValNames, valType, expr) => {
	  		var exprType = verify(expr, maps);
	  		if (checkTypeS(exprType, valType)) {
    	...
    	case VarDefStmt(listOfVarNames, varType, expr) => {
    		var exprType = verify(expr, maps);
	  		if (checkTypeS(exprType, varType)) {
    	...
    	case FunExpr(id, args) => {
    	    ...
    	    while (argTypes.length > 0 && paramTypes.length > 0) {
    	    	if (!checkTypeS(argTypes.head, paramTypes.head)){
    	...
    	case NumExpr(value) => {
    		...
    		return throw new Exception("Unknown numeric subtype: "+ast);
    	}
    	case BoolExpr(value) => "Boolean"
		case _ => throw new Exception("Unknown stmt: "+ast)
	}
	  	...
	  	case currentScope::rest => {
	  		if (currentScope.contains(funcName)) {
	  			...
	  			if (theType.isFunc()) {
	  				...
	  				return (theType.getRetType().getType()/*theType.getRetType()*/, argTypes.map(argType => argType.getType()));
	  			} else {
